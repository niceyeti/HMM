...
/*
@asExponentials: whether or not to raise e to the power of the matrix value or not,
eg, exp(matrix[i][j]). The use-case for this is if the matrix is stochastic, and holds
log-probabilities for numerical stability.
*/
template<typename T>
void Matrix<T>::Print(bool convertLogProbs)
{
	for(int i = 0; i < _matrix.size(); i++){
		for(int j = 0; j < _matrix[0].size(); j++){
			cout << setprecision(3);
			if(convertLogProbs && _matrix[i][j] <= 0)
				cout << exp(_matrix[i][j]) << " ";
			else
				cout << _matrix[i][j] << " ";
		}
		cout << endl;
...
This function is not preserving/stable; any previous contents of the matrix may
or may not be erased after resize.
*/
template<typename T>
